...
exports.makeWASocket = (connectionOptions, options = {}) => {
  ...
  conn.sendContact = async (jid, contact, quoted, info = {}, opts = {}) => {
    ...
    contact.map(v => list.push({
      displayName: v.name,
      vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:${v.name}\nORG:${info && info.org ? info.org : 'Bot Owner'}\nTEL;type=CELL;type=VOICE;waid=${v.number}:${PhoneNumber('+' + v.number).getNumber('international')}\nEMAIL;type=Email:${info && info.email ? info.email : 'itsyoshida@gmail.com'}\nURL;type=Website:${info && info.website ? info.website : 'https://bit.ly/m/mywebsite'}\nADR;type=Location:;;Unknown;;\nOther:${v.about}\nEND:VCARD`
    }))
